...
// ---------------------------------------------------------------------------
export const recursiveIdentityRuns = sqliteTable("recursive_identity_runs", {
  id: text("id").primaryKey().$defaultFn(() => nanoid()),
  agentId: text("agent_id").notNull().references(() => agents.id),
...
export const recursiveIdentityWorkers = sqliteTable("recursive_identity_workers", {
...
export const recursiveIdentityLedger = sqliteTable("recursive_identity_ledger", {
  id: text("id").primaryKey().$defaultFn(() => nanoid()),
  runId: text("run_id").notNull().references(() => recursiveIdentityRuns.id),
  agentId: text("agent_id").notNull().references(() => agents.id),
  ...
  responsePersisted: integer("response_persisted", { mode: "boolean" }).notNull().default(true),
  inputTokens: integer("input_tokens"),
  outputTokens: integer("output_tokens"),
  createdAt: integer("created_at", { mode: "timestamp" }).default(sql`(strftime('%s', 'now'))`),
}, (table) => [uniqueIndex("recursive_identity_ledger_run_iteration_idx").on(table.runId, table.iterationNumber)]);

export const recursiveIdentityFailures = sqliteTable("recursive_identity_failures", {
  id: text("id").primaryKey().$defaultFn(() => nanoid()),
  runId: text("run_id").notNull().references(() => recursiveIdentityRuns.id),
  agentId: text("agent_id").notNull().references(() => agents.id),
