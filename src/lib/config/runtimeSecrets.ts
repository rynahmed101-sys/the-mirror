...
function encryptionKey() {
  ...
  if (!value || value === "change-me-in-production") {
    throw new Error("Mirror runtime secret encryption key is not configured securely.");
  }
...
export async function clearRuntimeSecret(name: RuntimeSecretName) {
  await ensureTable();
  if (isPg) {
    await neonSql`DELETE FROM mirror_runtime_secrets WHERE name = ${name}`;
  } else {
    sqlite!.prepare(`DELETE FROM mirror_runtime_secrets WHERE name = ?`).run(name);
  }
}
...
export async function getRuntimeSecret(name: RuntimeSecretName) {
  ...
  try {
    if (isPg) {
      const rows = await neonSql`SELECT encrypted_value
        FROM mirror_runtime_secrets
        WHERE name = ${name}
        LIMIT 1`;
    ...
    const row = sqlite!.prepare(`SELECT encrypted_value
      FROM mirror_runtime_secrets
      WHERE name = ?
      LIMIT 1`).get(name) as { encrypted_value?: string } | undefined;
